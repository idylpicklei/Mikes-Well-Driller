...
## World-placed abandoned gas station. Walk-through Area2D — stand in/near, press E to search.
## Loot table is empty for now (PropLoot stub).

const PROMPT_OFFSET := Vector2(0, -72)
const INTERACT_PAD := Vector2(16, 8)
const EMPTY_FLASH_SEC := 1.15
const PROP_ID := &"gas_station"
...
func _ready() -> void:
	...
	if _sprite == null:
		...
		add_child(_sprite)
	_sprite.texture = PlaceholderGasStation.create_texture()
	_sprite.centered = true
	_sprite.position = PlaceholderGasStation.SPRITE_OFFSET
...
func _apply_interact_shape() -> void:
	...
	rect.size = Vector2(PlaceholderGasStation.SIZE) + INTERACT_PAD
	shape_node.shape = rect
	shape_node.position = PlaceholderGasStation.SPRITE_OFFSET
...
func _ensure_prompt() -> void:
	...
	_prompt.position = PROMPT_OFFSET + Vector2(-56, 0)
	_prompt.size = Vector2(112, 14)
